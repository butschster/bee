...
`bee.node.binding:describe({})` returns the current description and revision (zero before the
...
the native node ID. The selected resource updates the private `db.get` grant
with the database reference, while registry and native-identity reads remain
separate exact grants. Published methods retain the authenticated caller and
attach only the private store access needed for their implementation. The
`bee.node.traits:metadata_trait` registry entry grants no authority. Ordinary
applications cannot open the database.

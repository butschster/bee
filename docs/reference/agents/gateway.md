...
This change carries the queue and nothing that moves a byte to a child.

| Piece | State |
|---|---|
| Channel `queue` in `claims.CHANNELS`, its error text and capabilities test | written |
...
| `gateway_inbox` on a launch policy, opting one launch into downward delivery | written; no shipped policy sets it |
| Carrier drain, the harness adapters and the native client they need | **not here.** They follow as a separate change |

Nothing drains this queue yet, and that is deliberate rather than missing: a
row can be enqueued, claimed, marked dispatched and settled through the
operations above, but no carrier calls them and no adapter exists, so no
message has yet reached a child by this path. The transport is a separate
question — whether Bee may write into a child's cross-session inbox, and who
owns the socket address — and it is reviewed on its own. The measured
transport facts stay above because they are the reason the queue is durable at
all: two harnesses that differ by a whole turn in when they will accept a
message cannot be served by a direct hand-off.

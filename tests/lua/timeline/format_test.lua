...
local function define_tests()
    test.describe("Timeline format", function()
        ...
        test.it("summarizes messages, tools, turns and approvals from their own bodies", function()
            ...
            test.eq(message.state_label, "request")
            test.is_true(message.activity:find("hello", 1, true) ~= nil)
            ...
            test.is_true(started.details[1]:find("sequence 7  kind attempt.started  source bee", 1, true) ~= nil)
            test.is_true(started.details[3]:find("producer bee.test.producer  action act-1  attempt attempt-1  correlation corr-1", 1, true) ~= nil)
            ...
            test.is_true(child.details[3]:find("action child-action-", 1, true) ~= nil)
            test.is_true(child.details[3]:find("parent action parent-action", 1, true) ~= nil)
